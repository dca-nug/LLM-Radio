# ============================================================
# llm_harness_oneshot.py — Lengan LLM, kondisi ONE-SHOT.
# Sama dgn llm_harness.py TETAPI menyisipkan SATU contoh berlabel (sintetis,
# dibekukan, BUKAN dari korpus evaluasi) sebelum teks target.
# Output terpisah: LLM_predictions_oneshot.xlsx (kolom 'condition'='oneshot').
# Bandingkan dgn zero-shot untuk melihat efek prompting per model.
# ============================================================
...
OUTPUT_XLSX = DATA_DIR / 'LLM_predictions_oneshot.xlsx'
...
# ── CONTOH ONE-SHOT (SINTETIS, dibekukan, BUKAN dari korpus evaluasi) ──
# Mengajari: 'TB aktif' tertulis -> aktif; temuan disebut -> ada;
# temuan dinegasi ('tanpa') -> tidak; jantung normal -> kardiomegali tidak.
ONESHOT_TEXT = ("TB aktif. Tampak infiltrat dan nodul di apeks kanan, "
                "tanpa fibrosis maupun kavitas. Jantung normal.")
ONESHOT_JSON = {
    "alasan": "kesan menyebut 'TB aktif'; tampak infiltrat dan nodul; fibrosis dan kavitas dinegasi ('tanpa'); jantung normal",
    "is_radiologi": "radiologi", "imp_tb": "aktif",
    "infiltrat": "ada", "nodul": "ada", "kalsifikasi": "tidak",
    "fibrotik": "tidak", "kavitas": "tidak", "kardiomegali": "tidak",
}

PROMPT_HASH = hashlib.sha256(
    (SYSTEM_PROMPT + USER_TEMPLATE + json.dumps(SCHEMA, sort_keys=True)
     + ONESHOT_TEXT + json.dumps(ONESHOT_JSON, sort_keys=True)).encode()
).hexdigest()[:16]
...
def infer(name, teks):
    ...
    resp = ollama.chat(model=name,
        messages=[{'role':'system','content':SYSTEM_PROMPT},
                  {'role':'user','content':USER_TEMPLATE.format(teks=ONESHOT_TEXT)},
                  {'role':'assistant','content':json.dumps(ONESHOT_JSON, ensure_ascii=False)},
    ...
    for tag, name in MODELS:
        ...
        for rep in range(1, REPEAT + 1):
            for pos, r in src.iterrows():
                ...
                rec = {'ID_unik': r['ID_unik'], 'model': tag, 'condition': 'oneshot',
                       'repeat': rep,
